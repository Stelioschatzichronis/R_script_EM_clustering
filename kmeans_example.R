...
mean_Ar[[1,1]] = 1+mean(dataset[,1])
mean_Ar[[1,2]] = 1+mean(dataset[,2])

mean_Ar[[2,1]] = -1+mean(dataset[,1])
mean_Ar[[2,2]] = -1+mean(dataset[,2])

mean_Ar[[3,1]] = -1+mean(dataset[,1])
mean_Ar[[3,2]] = 1+mean(dataset[,2])
...
par(mfrow=c(2,2))
plot(dataset[,1],dataset[,2],col="green",xlim=c(-1.6,1.6),ylim=c(-1.7,1.7),pch=20,panel.first = grid(),xlab="x",ylab="y")
...
while(abs(J-previous_J)>0.01){
  ...
  plot(tr1[,1],tr1[,2],col="red",xlim=c(-1.6,1.6),ylim=c(-1.7,1.7),panel.first = grid(),xlab="x",ylab="y",main=toString(J))
...
plot(tr1[,1],tr1[,2],col="red",xlim=c(-1.6,1.6),ylim=c(-1.7,1.7),panel.first = grid(),xlab="x",ylab="y",main="using package k-means")
